...
export async function getExperts(querydata){
    const [start_date , end_date] = getDates(querydata)
    const { order , page , bankstatus } =  querydata
    try {
        let query , values ;
        ...
        //                     OFFSET $5 LIMIT $6`
        //             values = [status,start_date,end_date,"Tpp",post_per_page*(page-1),post_per_page]
        //         }else{
        //             query = `SELECT c.id as creator_id , c.full_name as name , c.email , c.created_at , c.status as account_status ,
        //                     (SELECT sum(payout_amount) FROM payments WHERE creator_id = c.id)  as payout_summary , 
        //                     (SELECT max(transaction_date) FROM payments WHERE creator_id = c.id) as last_payout_date , 
        ...
        //                     ORDER BY created_at ${order}
        //                     OFFSET $6 LIMIT $7`
        //             values = [status,bankstatus,start_date,end_date,"Tpp",post_per_page*(page-1),post_per_page]
        //         }
        ...
        //                     ORDER BY created_at ${order}
        //                     OFFSET $4 LIMIT $5`
        //             values = [start_date,end_date,"Tpp",post_per_page*(page-1),post_per_page]
        //         }else{
        //             query =`SELECT c.id as creator_id , c.full_name as name , c.email , c.created_at , c.status as account_status ,
        //                     (SELECT sum(payout_amount) FROM payments WHERE creator_id = c.id)  as payout_summary , 
        //                     (SELECT max(transaction_date) FROM payments WHERE creator_id = c.id) as last_payout_date , 
        ...
        //                     ORDER BY created_at ${order}
        //                     OFFSET $5 LIMIT $6`
        //             values = [bankstatus,start_date,end_date,"Tpp",post_per_page*(page-1),post_per_page]
        //         } 
        //     }else{
        //         query = 
        ...
        if(bankstatus === "Incomplete"){
            query = `SELECT c.id as creator_id , c.full_name as name , c.email , c.created_at , c.status as account_status ,
                     (SELECT sum(payout_amount) FROM tpp_payments WHERE creator_id = c.id)  as payout_summary , 
                     (SELECT max(transaction_date) FROM tpp_payments WHERE creator_id = c.id) as last_payout_date , 
                     cbd.verification_status as bank_details , c.phone_no
                     FROM creator AS c LEFT JOIN creator_bank_details AS cbd ON c.id = cbd.creator_id
                     WHERE cbd.verification_status IS NULL AND c.created_at BETWEEN $1 AND $2 AND c.creator_platform = $3
                     GROUP BY c.id , cbd.verification_status
            ...
            query =`SELECT c.id as creator_id , c.full_name as name , c.email , c.created_at , c.status as account_status ,
                    ...
                    cbd.verification_status as bank_details , c.phone_no
                    FROM creator AS c LEFT JOIN creator_bank_details AS cbd ON c.id = cbd.creator_id
                    WHERE c.created_at BETWEEN $1 AND $2 AND c.creator_platform = $3
                    GROUP BY c.id , cbd.verification_status
                    ORDER BY created_at ${order}
                    OFFSET $4 LIMIT $5`
            values = [start_date,end_date,"Tpp",post_per_page*(page-1),post_per_page]
...
export async function searchExpert(querydata){
